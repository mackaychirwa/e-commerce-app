...
const SampleTable: React.FC<SampleTableProps> = ({ columns, initialClaimsData = [], label, loading, columnNumber }) => {
    ...
    <div className={`mt-6 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-lg w-full">
          <div className={`grid grid-cols-4 sm:grid-cols-4 p-4 ${isDarkMode ? 'bg-gray-700' : 'bg-black'} text-white`}>
            {columns
              .filter(
                (column) =>
                  isSmallScreen
          ...
          {currentData.map((item, index) => (
            <div
              key={index}
              className={`p-2 sm:p-4 mb-[1px] grid grid-cols-4 sm:grid-cols-4 gap-2 sm:gap-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
            >
              {columns
                .filter(
                  (column) =>
