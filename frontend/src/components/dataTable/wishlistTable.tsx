...
interface NewProduct {
  ...
  product?: { id: number; name: string, unit_cost: number, qty: number};
}
...
const WishlistTable: React.FC<SampleTableProps> = ({ columns, initialClaimsData = [], label, loading, columnNumber, token, category }) => {
      ...
      render: (_: any, item: NewProduct) => (
        <div className="flex space-x-2">
          {item.review_state === 1 ? (
            ...
            <button onClick={() => handleReply(item.id)} className="px-2 py-1 bg-green-500 text-white rounded">Notify</button>
          )}
        </div>
      ),
    },
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-lg w-full">
          ...
          {currentData.map((item, index) => (
            <div
              key={index}
              className={`p-2 sm:p-4 mb-[1px] grid grid-cols-4 sm:grid-cols-5 gap sm:gap-5 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
            >
              {columns.map((column) => (
                <div key={column.key}>
                  ...
                  ) : column.dataIndex === 'unit_cost' ? (
                    <span>{item?.product?.unit_cost || 'No unit_cost found'}</span>
                  ) : column.dataIndex === 'qty' ? (
                    <span>{item?.product?.qty || 'No qty found'}</span>
                  ) : column.render ? (
